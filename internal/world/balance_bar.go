...
// Update ничего не делает, но должен быть реализован для интерфейса Layer.
func (b *BalanceBarLayer) Update(ctx WorldContext) {
	// можно добавить анимацию в будущем
}

// Draw рисует полоску баланса.
func (b *BalanceBarLayer) Draw(screen *ebiten.Image, cam *render.Camera, ctx WorldContext) {
	if b.isFalling() {
		return
	}
